...
document.addEventListener("DOMContentLoaded", function () {
    ...
    thumbWraps.forEach(function (thumbWrap) {
        thumbWrap.addEventListener("click", function (e) {
            ...
            const vimeoIdMatch = videoUrl.match(
                /vimeo\.com\/(?:video\/)?(\d+)/,
            );
            ...
            // Find the sibling h2 (looks at the video's parent, then finds an h2 within)
            const h2 = videoWrapper.parentElement.querySelector("h2");

            if (!h2) {
                console.warn("No sibling h2 element found");
                return;
            }

            // Fetch video metadata from Vimeo's oEmbed endpoint
            fetch(
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!grid || !loadBtn) {
        console.warn("Missing .thumbs-grid or #btn-load");
        return;
    }

    const thumbs = Array.from(grid.querySelectorAll(".thumb-wrap"));

    // Hide everything past the initial count
    ...
    // Hide the button if there's nothing to load
    ...
    loadBtn.addEventListener("click", function (e) {
        ...
        const nextBatch = thumbs.slice(visibleCount, visibleCount + BATCH_SIZE);

        // Reveal the batch but keep them invisible for GSAP to animate from
        ...
        // Hide the button when everything is revealed
        if (visibleCount >= thumbs.length) {
            loadBtn.style.display = "none";
        }
    });
});
